var productList = [
  ...
  heading.append("Clothing for Men and Women")
  const parentDiv= document.createElement('div')
  content.appendChild(parentDiv)
  parentDiv.style.display="flex"
  parentDiv.style.justifyContent= "center"
//creating divs
  function cardAll(num,val){
   for(i=0;i<num;i++){
    let x=document.createElement('div')
    val.append(x)
    x.classList.add("mycards")
   }
  }


//uploading images to div
function imagesForCards(num){
  for(i=0;i<num;i++){
    let y=document.createElement('img')
    y.setAttribute("src",productList[i].preview)
    y.setAttribute("width","260px")
    y.setAttribute("height","350px")
    allDiv[i].append(y)
  }
}

//creating name for product
function titleForCards(num){
  for(i=0;i<num;i++){
    let x=document.createElement('h3')
    allDiv[i].append(x)
    x.append(productList[i].name)
  }
}


//adding brand name
function brandForCards(num){
  for(i=0;i<num;i++){
    let x=document.createElement('h4')
    allDiv[i].append(x)
    x.append(productList[i].brand)
  }
}


//adding price to product
function priceForCards(num){
  for(i=0;i<num;i++){
    let x=document.createElement('h5')
    allDiv[i].append(x)
    x.append("Rs " + productList[i].price)
  }
}
...
heading2.append("Accessories for Men and Women")
const parentDiv2= document.createElement('div')
content.appendChild(parentDiv2)
parentDiv2.style.display="flex"
parentDiv2.style.justifyContent= "center"

//calling cards
cardAll(5,parentDiv2)
const allDiv= document.querySelectorAll('.mycards')
imagesForCards(10)
titleForCards(10)
brandForCards(10)
priceForCards(10)